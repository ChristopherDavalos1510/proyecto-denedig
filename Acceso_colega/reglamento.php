...
$nombre_archivo_original = $_FILES['reglamento']['name'];
$extension_archivo = pathinfo($nombre_archivo_original, PATHINFO_EXTENSION);
$nombre_archivo_nuevo = "Reglamento_" . $id_usuario . '_' . str_replace(" ", " ", $nombre_completo) . '.' . $extension_archivo;
$archivo_temporal_pdf = $_FILES['reglamento']['tmp_name'];
...
if (move_uploaded_file($archivo_temporal_pdf, $destino_pdf)) {
    ...
    $sql = "UPDATE usuarios SET reglamento = 1, reglamento_link = ? WHERE id_usuario = ?";
    $stmt = $conexion->prepare($sql);
    $reglamento_link = '../documentos/' .$id_usuario . "/Reglamento_" . $id_usuario . '_' . str_replace(" ", " ", $nombre_completo) . '.' . $extension_archivo;
    $stmt->bind_param("si", $reglamento_link, $id_usuario);
    $stmt->execute();
    $stmt->close();
    // Redirige a la página de confirmación después de subir el reglamento
    header("Location: reglamento_subido.php");
    exit();
} else {
    echo "";
}
...
$select_sql = "SELECT reglamento FROM usuarios WHERE id_usuario = ?";
$stmt_select = $conexion->prepare($select_sql);
$stmt_select->bind_param("i", $id_usuario);
$stmt_select->execute();
$stmt_select->bind_result($reglamento_subido);
$stmt_select->fetch();
$stmt_select->close();

if ($reglamento_subido == 1) {
...
<main>
    <section class="hero-section align-items-center d-flex justify-content-center " style="background-color: #142e61ff; color: #fff; margin-top: -1450px;">
        <div class="container text-center" style= "margin-left: 180px;"> <br>
            <h1 style="color: #fff; margin-left: -160px;" class="mb-3">Reglamento</h1>
             <div style="margin-top: -30px; "class="container py-5">
            <h style="color: #fff; margin-left: -200px;""2>Sube tu reglamento firmado</h2>
            <form action="reglamento.php" method="post" enctype="multipart/form-data">
                            <div class="mb-3">
                        <label style="color: #fff; margin-left: -200px;" for="reglamento" class="form-label">Selecciona el archivo de Reglamento  (PDF) (máx. 500KB):</label>
                        <input style="margin-left: 50px; width: 500px;" type="file" class="form-control" id="reglamento" name="reglamento" accept=".pdf" onchange="validarArchivo(this, 'errorSalud')" required>
                         <span id="errorSalud" style="color: red; margin-left: -200px;"></span>
                    </div>
                    <button type="submit" class="btn btn-primary" style="background-color: #f65129; border-color: #f65129; width: 200px; height: 50px; font-size: 22px; margin-left: -160px;" >Subir Documentos</button>
        ...
        <div class="container py-5" style= "margin-top: 60px;">
            <iframe src="../reglamentos/Reglamento Denedig 24.pdf" margin-left = "1000px !important"; width="60%" height="700px" style="border:none;"></iframe>
            <br>
        </div>
    </section>
